...
// osm tiles draw de facto borders: kashmir carved up by dashed disputed
// lines plus de facto ind/pak/chn lines, which is not the depiction mandated
// in india. hide the dashes, hide every tile border between these three
// countries, and overlay the india-pov set (natural earth FCLASS_IN,
// generated by scripts/build-india-borders.mjs) styled exactly like the
...
function applyIndianBorders(map: maplibregl.Map) {
  ...
  if (map.getLayer("boundary_2")) {
    map.setFilter("boundary_2", [
      "all",
      map.getFilter("boundary_2"),
      [
        "any",
        ["!", ["in", ["coalesce", ["get", "adm0_l"], ""], HIDDEN_PAIR]],
        ["!", ["in", ["coalesce", ["get", "adm0_r"], ""], HIDDEN_PAIR]],
      ],
    ] as unknown as maplibregl.FilterSpecification);
    // below z5 the tiles merge all borders into per-tile blobs with no
    // country codes or disputed flags (the loc gets baked in), so the pair
    // filter can't work there. hide them and draw the low:1 world set instead
    map.setLayerZoomRange("boundary_2", 5, 24);
  }
  if (map.getSource("india-claim")) return;
  map.addSource("india-claim", {
    type: "geojson",
    data: indiaClaim as unknown as GeoJSON.FeatureCollection,
  });
...
<svg xmlns="http://www.w3.org/2000/svg" width="${size}" height="${size}" viewBox="0 0 40 40" style="display:block;overflow:visible;">
  <g data-bearing="" style="filter: drop-shadow(rgba(0, 0, 0, 0.35) 0px 2px 3px); transform-box: fill-box; transform-origin: 50% 50%;">
...
interface Options {
  ...
  // fires on user-initiated pan/zoom (not programmatic moves); hosts use it
  // to pause the scroll-follow camera
  onUserInteract?: () => void;
...
export function createRouteMap(opts: Options): RouteMapHandle {
  ...
  // user-initiated gestures carry originalEvent; programmatic jumpTo doesn't
  if (opts.onUserInteract) {
    const markUser = (e: { originalEvent?: unknown }) => {
      if (e.originalEvent) opts.onUserInteract?.();
    };
  ...
  return {
    ...
    follow(center, followZoom, snap = false) {
      // a far off-screen target (anchor jump, find-in-page) also snaps:
      // gliding 18% per scroll event would strand the car off-screen after
      // a single-event jump
      const el = map.getContainer();
      const p = map.project(center);
      const far =
        Math.hypot(p.x - el.clientWidth / 2, p.y - el.clientHeight / 2) >
        1.2 * Math.max(el.clientWidth, el.clientHeight);
      if (snap || far) {
        map.jumpTo({ center, zoom: followZoom });
        return;
      }
      const z = map.getZoom();
      const zoom =
        Math.abs(z - followZoom) < 0.05
          ? followZoom
          : z + (followZoom - z) * 0.18;
      const c = map.getCenter();
      const dLng = center[0] - c.lng;
      const dLat = center[1] - c.lat;
      const close = Math.abs(dLng) < 1e-5 && Math.abs(dLat) < 1e-5;
      map.jumpTo({
        center: close ? center : [c.lng + dLng * 0.18, c.lat + dLat * 0.18],
        zoom,
      });
    },
    destroy() {
